...
export class MiningMockDataSource implements MiningDataSource {
  ...
  private animateRobot(time: number, progress: number) {
    ...
    store.robot.jointVelocity = store.robot.jointNames.map((_, index) => Math.cos(phase + index * .55) * .12); store.appendJointPositionSample({timestamp:store.robot.timestamp,positions:[...store.robot.jointPosition],velocities:[...store.robot.jointVelocity]}); store.robot.jointEffort = store.robot.jointNames.map((_, index) => 3.5 + Math.sin(time / 800 + index) * .8)
