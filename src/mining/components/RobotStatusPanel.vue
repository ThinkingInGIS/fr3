...
const render = () => {
  ...
  chart.setOption({
    ...
    series: [
      series('Fx', '#FFFF00', data.map(item => item.force.x), 0), series('Fy', '#00FF00', data.map(item => item.force.y), 0), series('Fz', '#FF00FF', data.map(item => item.force.z), 0),
    ],
  }, true)
}
  ...
  <section class="mine-panel robot-status-card">
    <div class="mine-panel-head compact">
      ...
      <div class="robot-status-tools"><b :class="['controller-state', store.robot.controllerState.toLowerCase()]">{{
        stale ? 'STALE' : store.robot.controllerState }}</b><span class="wrench-unit">末端三轴力 · N</span></div>
    </div>
