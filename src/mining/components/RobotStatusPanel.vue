...
<template>
  <section class="mine-panel robot-status-card">
    <div class="mine-panel-head compact"><div><h2>实时作业状态</h2></div><div class="robot-status-tools"><b :class="['controller-state',store.robot.controllerState.toLowerCase()]">{{ stale?'STALE':store.robot.controllerState }}</b><button @click="unit=unit==='deg'?'rad':'deg'">{{ unit==='deg'?'°':'rad' }}</button><button @click="expanded=!expanded"><ChevronDown :class="{rotated:expanded}" :size="13" /></button></div></div>
    ...
    <div class="robot-live-joints" aria-label="关节角" :style="{gridTemplateColumns:`auto repeat(${Math.max(1,store.robot.jointNames.length)}, minmax(0, 1fr))`}">
      <h5>关节角</h5>
      <div v-for="(name,index) in store.robot.jointNames" :key="name" :title="name"><b>J{{ index+1 }}</b><strong>{{ joint(store.robot.jointPosition[index]??0) }}</strong></div>
    </div>
