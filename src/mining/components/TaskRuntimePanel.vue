...
<template>
  <section class="mine-panel runtime-card">
    <div class="mine-panel-head compact"><div><h2>作业信息</h2></div><em>{{ store.workflow.stage }}</em></div>
    <div class="runtime-top"><div><Clock3 :size="14" /><span>总执行时间</span><strong>{{ format(elapsed) }}</strong></div><div><BrainCircuit :size="14" /><span>规划耗时</span><strong>{{ planningDuration??'—' }}<small v-if="planningDuration!==undefined"> ms</small></strong></div><div><Target :size="14" /><span>剩余子任务</span><strong>{{ store.remainingTasks }}</strong></div></div>
    <div class="runtime-info"><div><span>当前任务</span><strong>{{ currentTaskTitle??'等待任务' }}</strong></div><div><span>当前对象</span><strong><Crosshair :size="12" />{{ currentObjectId??'—' }}</strong></div><div class="wide"><span>决策说明</span><strong>{{ decisionReasons.join(' · ')||'等待意图理解结果' }}</strong></div></div>
    <div class="runtime-progress"><span>整体进度</span><i><b :style="{width:`${overallProgress}%`}" /></i><strong>{{ overallProgress.toFixed(0) }}%</strong><em>{{ lastResult }}</em></div>
  </section>
</template>
