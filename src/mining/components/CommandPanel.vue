...
const source=getMiningDataSource(),store=useMiningStore(),command=ref('开始钻锚作业'),error=ref(''),sending=ref(false)
const canSend=computed(()=>command.value.trim()&&!store.running)
const send=async()=>{if(!canSend.value)return;error.value='';sending.value=true;try{if(source.mode==='ros'&&miningConfig.commandConfirm&&!window.confirm(`确认发送真实作业指令：${command.value.trim()}？`))return;await source.sendCommand(command.value)}catch(e){error.value=e instanceof Error?e.message:'指令发送失败'}finally{sending.value=false}}
...
<template>
  <section class="mine-panel command-card">
    ...
    <div class="command-actions"><button class="send-command" :disabled="!canSend||sending" @click="send"><Send :size="14" />{{ sending?'发送中':'发送指令' }}</button><span>Enter 发送 · Shift+Enter 换行</span></div>
    ...
    <p class="safety-note">“取消”仅发送任务取消请求，不是硬件安全急停。</p>
  </section>
</template>
